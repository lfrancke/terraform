...
func (b *Remote) plan(stopCtx, cancelCtx context.Context, op *backend.Operation, w *tfe.Workspace) (*tfe.Run, error) {
	...
	r, err = b.waitForRun(stopCtx, cancelCtx, op, r, w)
	if err != nil {
		return r, err
	}
...
func (b *backoffPolicy) Duration(i int) time.Duration {
	if i >= len(b.delays) {
		i = len(b.delays) - 1
	}
	return time.Duration(b.delays[i]) * time.Second
}

var defaultBackoff = &backoffPolicy{
	delays: []int{1, 2, 3, 4, 5},
}

func (b *Remote) waitForRun(stopCtx, cancelCtx context.Context, op *backend.Operation, r *tfe.Run, w *tfe.Workspace) (*tfe.Run, error) {
	started := time.Now()
	updated := started
	for i := 0; ; i++ {
		select {
		...
		case <-time.After(defaultBackoff.Duration(i / 5)):
		...
		if r.Status != tfe.RunPending {
			if i == 0 && b.CLI != nil {
				b.CLI.Output(b.Colorize().Color("Waiting for the run to start..."))
			}
			return r, nil
		}
		...
		search:
			...
			b.CLI.Output(b.Colorize().Color("Waiting for the run to start..." + elapsed))
		}
	}
}
